...
def arquivoOK(text, resp):
    _conv = open('Email.txt', 'a', encoding='utf-8')
    _conv.writelines(f"Email: {text},\n Tirulo: {resp}\n\n")
    _conv.close()
    ...
    for response_part in data:
        ...
        if isinstance(response_part, tuple):
            ...
            if message.is_multipart():
                ...
                continue # mail_content = message.get_payload()

            # por fim vamos mostrar na tela o resultado da extração

            if mail_from == 'noreply@salesforce.com':
                mail_content = bs(mail_content, 'html.parser')
                arquivoOK(mail_from, mail_subject)
                print(f'From: {mail_from}')
                print(f'Subject: {mail_subject}')
                # print(f'Content: {mail_content.text}')
